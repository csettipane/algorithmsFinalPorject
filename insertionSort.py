...
class InsertionSort():
    ...
    def sort(self):
        for i in range(0, len(self.list)):
            key = self.list[i]
            j = i-1
            self.drawRect(i, len(self.list)+1, len(self.list)+1)
            while j >= 0 and key < self.list[j] :
                self.list[j+1] = self.list[j]
                j -= 1
                self.list[j+1] = key
                self.drawRect(j+1, j, len(self.list)+1)
        self.drawRect(len(self.list)+1, len(self.list)+1 -1)
